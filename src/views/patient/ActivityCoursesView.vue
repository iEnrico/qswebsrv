  ...
  <v-card class="mx-6 mt-6 rounded-lg" variant="elevated">
    <v-row class="ma-4" style="align-items: center; justify-items: center">
      <v-card-title class="ma-0 pa-0 mr-2">{{
        $t("description_dashboard_activity_courses")
      }}</v-card-title>
      <v-btn
        style="height: 24px"
        variant="text"
    ...
    <ActivitesList v-if="!loading" :searchText="searchText" :items="activities"/>
  </v-card>
...
export default {
  ...
  methods: {
    getActivities: async function () {
      ...
      this.activities = activitiesResult.data.filter(function (o) {
        return o.primaryType == 'WEBSITE' ||
